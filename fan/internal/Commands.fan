...
class Commands {
	...
	** We use a Str for cmdUrl so we get the *exact* text and not some URI standard form approximation of. 
	Void doCmd(FixtureCtx fixCtx, Str cmdUrl, Str cmdText, Str[][]? table, Int? tableRowIdx, Int? tableColIdx) {
		...
		try {
			...
			if (ignore && command.canFailFast)
				...
				command.runCommand(fixCtx, CommandCtx(cmdScheme, cmdPath, cmdText, table, tableRowIdx, tableColIdx, ignore))
			}
